"""
实验2：使用d-rag系统的情感分析实验
调用d-rag系统进行增强的情感分析
支持断点重连和后台运行
增强版：支持从指定位置继续、GLM4断连检测、定期保存
"""
...
DATA_FILE = "data4.csv"
OUTPUT_FILE = "results_with_rag.csv"
LOG_FILE = "experiment_with_rag.log"
CHECKPOINT_FILE = "experiment_with_rag_checkpoint.json"
CONSOLE_LOG_FILE = "experiment_with_rag_console.log"

# ============ 断点续传配置 ============
# 从哪个索引开始处理（0表示从头开始，设置为2109表示跳过前2109条已处理的记录）
START_INDEX = 0
# 每处理多少条保存一次结果
SAVE_INTERVAL = 10
# 连续多少条预测为0视为GLM4断连
...
4. **分析过程要求 (CoT)**：
   在输出最终标签前，请进行一步**【冲突检查】**：
   - "我检索到的背景信息是针对本次具体事件的吗？如果是历史旧账或标准免责声明，我是否已经排除了它的干扰？"

### 输出说明
如果使用了检索，请简述理由；如果检索内容被判定为无效噪声或历史包袱，请明确注明"忽略无效检索"。
"""
...
def run_experiment():
    ...
    log_message("=" * 80)
    log_message("实验2：使用d-rag系统的情感分析（增强版：断点续传+断连检测）")
    log_message("=" * 80)
    ...
    log_file_handle.write(f"继续实验: {datetime.now()}\n")
