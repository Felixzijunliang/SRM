"""
实验2变体：使用混合检索(Hybrid RAG)系统的情感分析实验（无CoT版本）
用于消融实验：使用语义+关键词+RRF融合+Reranker的混合检索，不使用Chain-of-Thought推理
...
DATA_FILE = "data4.csv"
OUTPUT_FILE = "results_with_hybid_rag_without_cot.csv"
LOG_FILE = "experiment_with_hybid_rag_without_cot.log"
CHECKPOINT_FILE = "experiment_with_hybid_rag_without_cot_checkpoint.json"
CONSOLE_LOG_FILE = "experiment_with_hybid_rag_without_cot_console.log"
...
def run_experiment():
    ...
    log_message("=" * 80)
    log_message("实验2变体：使用混合检索(Hybrid RAG)的情感分析（无CoT版本 - 消融实验）")
    ...
    log_file_handle.write(f"继续实验（混合检索Hybrid RAG + 无CoT版本）: {datetime.now()}\n")
