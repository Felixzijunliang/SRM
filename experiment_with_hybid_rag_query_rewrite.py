"""
实验2变体：使用混合检索(Hybrid RAG) + 查询改写的情感分析实验（无CoT版本）
用于消融实验：使用语义+关键词+RRF融合+Reranker的混合检索 + Qwen3-4B查询改写，不使用Chain-of-Thought推理
通过llm_service(use_rag=True)调用，由GLM-4 Tool Calling自行判断是否检索。
后端hybrid_rag_api提供混合检索（第一阶段召回~25个候选，第二阶段Reranker保留Top3）。
查询改写：通过Qwen3-4B进行上下文改写和多查询扩展，提升检索召回率。
从llm_service返回的tool_result中提取混合检索详细分数。
"""
...
DATA_FILE = "data4.csv"
OUTPUT_FILE = "results_with_hybid_rag_query_rewrite.csv"
LOG_FILE = "experiment_with_hybid_rag_query_rewrite.log"
CHECKPOINT_FILE = "experiment_with_hybid_rag_query_rewrite_checkpoint.json"
CONSOLE_LOG_FILE = "experiment_with_hybid_rag_query_rewrite_console.log"
...
def _extract_hybrid_scores_from_tool_result(tool_result_str):
    """
    从llm_service返回的tool_result中提取混合检索详细分数

    tool_result是JSON字符串，格式：
    {"success": true, "retrieved_count": N, "context": "...",
     "documents": [{"text": "...", "score": 0.8, "source": "...",
                     "semantic_score": 0.7, "keyword_score": 0.5,
                     "rrf_score": 0.6, "reranker_score": 0.8}, ...]}
    """
    ...
    try:
        ...
        documents = tool_data.get('documents', [])
    ...
    return hybrid_scores, top_sources


def _extract_score(result, index, score_key):
    """从hybrid_scores中提取指定索引的指定分数"""
    scores = result.get('hybrid_scores', [])
    if index < len(scores):
        return scores[index].get(score_key)
    return None


def call_llm_with_hybrid_rag(text, prompt_template):
    """调用LLM服务（带混合RAG + 查询改写，无CoT）进行情感分析

    use_rag=True，由GLM-4 Tool Calling自行判断是否需要检索。
    后端hybrid_rag_api提供混合检索（语义+关键词+RRF+Reranker）。
    查询改写：通过Qwen3-4B进行上下文改写和多查询扩展。
    从返回的tool_result中提取混合检索详细分数。
    """
    # 截断过长文本
...
{RAG_ENHANCEMENT}"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            tool_result_str = result.get('tool_result', '')
            retrieved_count = result.get('retrieved_count', 0)

            # 提取查询改写信息
            rewrite_info = result.get('rewrite_details', {})
            rewrite_enabled = rewrite_info.get('enabled', False)
            rewritten_queries = rewrite_info.get('rewritten_queries', [])
            resolved_query = rewrite_info.get('resolved_query', '')

            # 从tool_result提取混合检索分数
            hybrid_scores, top_sources = _extract_hybrid_scores_from_tool_result(tool_result_str)
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                return {
                    ...
                    "rewrite_count": len(rewritten_queries) if rewritten_queries else 0
                }
            else:
...
def check_services():
    ...
    try:
        ...
        log_message(f"✓ 混合检索RAG API 正常运行" if response.status_code == 200 else f"✗ 混合检索RAG API 状态异常")
    except:
        log_message(f"✗ 混合检索RAG API 无法连接，请启动: cd /home/turing/d-rag && python hybrid_rag_api.py")
        return False
...
def wait_for_api_recovery():
    ...
    while time.time() - wait_start < API_MAX_WAIT_TIME:
        ...
        try:
            rag_ok = requests.get(f"{RAG_API_URL}/health", timeout=5).status_code == 200
            llm_ok = requests.get("http://localhost:8889/health", timeout=5).status_code == 200

            if rag_ok and llm_ok:
                log_message("✓ 混合检索RAG API和LLM服务均已恢复，继续处理...")
                return True
            else:
                status = []
                if not rag_ok:
                    status.append("混合检索RAG API不可用")
                if not llm_ok:
                    status.append("LLM服务不可用")
                log_message(f"部分服务仍不可用: {', '.join(status)}，继续等待...")
        except:
...
def run_experiment():
    ...
    log_message("=" * 80)
    log_message("实验2变体：使用混合检索(Hybrid RAG) + 查询改写的情感分析（无CoT版本 - 消融实验）")
    log_message("混合检索：语义(BGE-M3) + 关键词(BM25) + RRF融合 + Reranker精排")
    log_message("查询改写：Qwen3-4B上下文改写 + 多查询扩展")
    log_message("检索策略：~25候选 → Reranker → Top3 给LLM | LLM自行判断是否检索")
    log_message("=" * 80)
    ...
    log_file_handle.write(f"继续实验（混合检索Hybrid RAG + 查询改写 + 无CoT版本）: {datetime.now()}\n")
    log_file_handle.write(f"从索引 {START_INDEX} 开始处理\n")
    log_file_handle.write(f"检索策略: ~25候选 → Reranker → Top3 | LLM自行判断是否检索\n")
    log_file_handle.write(f"查询改写: Qwen3-4B上下文改写 + 多查询扩展\n")
    ...
    for idx in tqdm(range(START_INDEX, len(df)), desc="处理进度", initial=0, total=total_to_process):
        ...
        # 调用混合检索+查询改写+LLM
        result = call_llm_with_hybrid_rag(row['text'], prompt_template)
        retry_count = 0
        max_retries = 3
        ...
        result_record = {
            ...
            'hybrid_scores': json.dumps(result.get('hybrid_scores', []), ensure_ascii=False),
            'top1_semantic_score': _extract_score(result, 0, 'semantic_score'),
            'top1_keyword_score': _extract_score(result, 0, 'keyword_score'),
            'top1_rrf_score': _extract_score(result, 0, 'rrf_score'),
            'top1_reranker_score': _extract_score(result, 0, 'reranker_score'),
            'top1_final_score': _extract_score(result, 0, 'final_score'),
            'top2_semantic_score': _extract_score(result, 1, 'semantic_score'),
            'top2_keyword_score': _extract_score(result, 1, 'keyword_score'),
            'top2_rrf_score': _extract_score(result, 1, 'rrf_score'),
            'top2_reranker_score': _extract_score(result, 1, 'reranker_score'),
            'top2_final_score': _extract_score(result, 1, 'final_score'),
            'top3_semantic_score': _extract_score(result, 2, 'semantic_score'),
            'top3_keyword_score': _extract_score(result, 2, 'keyword_score'),
            'top3_rrf_score': _extract_score(result, 2, 'rrf_score'),
            'top3_reranker_score': _extract_score(result, 2, 'reranker_score'),
            'top3_final_score': _extract_score(result, 2, 'final_score'),
            'rewrite_enabled': result.get('rewrite_enabled', False),
            'rewrite_count': result.get('rewrite_count', 0),
            'resolved_query': result.get('resolved_query', ''),
            'rewritten_queries': json.dumps(result.get('rewritten_queries', []), ensure_ascii=False),
        ...
        if result['success']:
            log_file_handle.write(f"RAG使用:{result.get('used_rag')} 检索:{result.get('retrieved_count', 0)}篇\n")
            if result.get('rewrite_enabled'):
                log_file_handle.write(f"查询改写: {result.get('rewrite_count', 0)}个查询\n")
                log_file_handle.write(f"  改写后查询: {result.get('resolved_query', '')}\n")
            for hs in result.get('hybrid_scores', []):
                log_file_handle.write(
                    f"  Doc{hs['rank']}: semantic={hs.get('semantic_score', 0):.4f} "
                    f"keyword={hs.get('keyword_score', 0):.4f} "
                    f"rrf={hs.get('rrf_score', 'N/A')} "
                    f"reranker={hs.get('reranker_score', 'N/A')} "
                    f"final={hs.get('final_score', 0):.4f}\n"
                )
        else:
    ...
    rewrite_used = results_df['rewrite_enabled'].sum() if 'rewrite_enabled' in results_df.columns else 0
    ...
    log_message(f"成功: {success_count}/{len(results)} | RAG使用: {rag_used}/{len(results)} | 查询改写: {rewrite_used}/{len(results)} | 准确率: {accuracy*100:.2f}%")
    ...
    log_file_handle.write(f"RAG使用: {rag_used}/{len(results)}\n")
    log_file_handle.write(f"查询改写: {rewrite_used}/{len(results)}\n")
    log_file_handle.close()
